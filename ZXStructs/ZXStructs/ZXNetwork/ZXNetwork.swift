...
class ZXNetwork: NSObject {
    ...
    private final class func commonProcess(data:Data?,url:String,completion:ZXHTTPCompletionAction?) {
    ...
    private final class func httpError(error:Error,url:String,timeout:ZXHTTPTimeOutAction?,httpError:ZXHTTPErrorAction?) {
        if showRequestLog {
    ...
    @discardableResult final class func asyncRequest(withUrl url:String,params:Dictionary<String, Any>?,method:ZXHTTPMethod,completion:ZXHTTPCompletionAction?,timeOut:ZXHTTPTimeOutAction?,httpError:ZXHTTPErrorAction?) -> URLSessionTask? {
        if showRequestLog {
    ...
    ///   - httpError: http request error
    /// - Returns: return value description
    @discardableResult class func uploadImage(to url:String!,images:Array<UIImage>!,params:Dictionary<String,Any>?,compressRatio:CGFloat,completion:ZXHTTPCompletionAction?,timeOut:ZXHTTPTimeOutAction?,httpError:ZXHTTPErrorAction?) -> URLSessionTask? {
        if showRequestLog {
            print("\n------------Request------------\n请求地址:\n\(url)\n请求参数:\n\(String(describing: params))\n---------------------------\n")
        }
        var paramsString: String?
